...
volatile uint32_t flash_write_done = 0;
volatile uint32_t pending_flash_op = 0;
volatile uint32_t pending_write_addr = 0;
volatile uint32_t pending_write_len = 0;
uint8_t write_buffer [32768];
...
{
  /* USER CODE BEGIN 7 */
    // this function runs when the "run after programming" bit has been set.
    // it finalizes the
    extern uint32_t new_reset_vect;
...
{
  /* USER CODE BEGIN 9 */
    if (pending_flash_op == 0) {
        memcpy(write_buffer, src, Len);
        pending_flash_op = 1;
        pending_write_addr = (uint32_t)dest;
